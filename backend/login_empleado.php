...
try {
    ...
    $sql = "SELECT e.ciEmpleado, e.nombre1, e.nombre2, e.apellidoP, e.apellidoM, 
                   e.correo, e.idRol, r.nombreRol
            FROM templeados e
    ...
    if ($usuario) {
        echo json_encode([
            ...
            "usuario" => [
                ...
                "nombre"     => trim($usuario['nombre1'] . ' ' . ($usuario['nombre2'] ?? '') . ' ' . $usuario['apellidoP'] . ' ' . ($usuario['apellidoM'] ?? '')),
